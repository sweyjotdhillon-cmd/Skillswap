...
export function LoginPage({ onNavigate, redirectTo }: LoginPageProps) {
  ...
  useEffect(() => {
    ...
    if (error || errorDescription) {
      let friendlyMsg = 'Authentication failed or was cancelled. Please try again.';
      const rawDesc = (errorDescription || error || '').toLowerCase();
      if (rawDesc.includes('cancel') || rawDesc.includes('access_denied')) {
        friendlyMsg = 'Authentication was cancelled. Please try again when you are ready.';
      } else if (rawDesc.includes('email')) {
        friendlyMsg = 'Unable to retrieve your email address from the provider. Please ensure your provider account has a verified primary email address.';
  ...
  const handleGitHubSignIn = async () => {
    ...
    try {
      ...
      const msg = (err as Error)?.message || 'An unexpected error occurred during GitHub sign in.';
      setErrorMessage(msg);
    }
  };
    ...
    <div className="page-shell">
      ...
      <main className="auth-layout-grid">
        ...
        <section className="auth-card-container">
          ...
          <div className="auth-oauth-group" style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginBottom: '1.5rem' }}>
            ...
            <button type="button" className="auth-github-btn" onClick={handleGitHubSignIn}>
