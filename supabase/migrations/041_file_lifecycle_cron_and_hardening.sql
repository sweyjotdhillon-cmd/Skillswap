  ...
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron') THEN
    ...
    IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_net') THEN
      EXECUTE 'SELECT cron.schedule('
        ...
        || quote_literal('*/10 * * * *') || ', '
        || quote_literal('SELECT net.http_post(url := COALESCE(current_setting(''app.settings.edge_function_base_url'', true), ''http://127.0.0.1:54321/functions/v1'') || ''/file-lifecycle-worker'', headers := jsonb_build_object(''Content-Type'', ''application/json'', ''Authorization'', ''Bearer '' || COALESCE(current_setting(''app.settings.service_role_key'', true), '''')), body := jsonb_build_object(''batch_size'', 500));')
        || ')';
    END IF;
  END IF;
