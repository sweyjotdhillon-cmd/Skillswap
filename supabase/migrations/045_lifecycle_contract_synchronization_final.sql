...
BEGIN
  ...
  IF p_storage_path IS NULL OR p_storage_path NOT LIKE 'swap-chat-attachments/' || v_msg.swap_id::text || '/' || v_user::text || '/%' THEN
...
-- Function 5: Atomic send_chat_message_with_attachments (Updated for canonical swap-chat-attachments path)
...
BEGIN
  ...
  IF v_att_count > 0 THEN
    FOR v_att IN SELECT * FROM jsonb_to_recordset(p_attachments) AS x(storage_path text, file_name text, mime_type text, file_size bigint)
    LOOP
      v_storage_path := trim(COALESCE(v_att.storage_path, ''));
      v_file_name := trim(COALESCE(v_att.file_name, ''));
      v_file_size := v_att.file_size;

      IF v_storage_path IS NULL OR v_storage_path NOT LIKE 'swap-chat-attachments/' || p_swap_id::text || '/' || v_user::text || '/%' THEN
        RAISE EXCEPTION 'Invalid storage path structure for chat attachment.';
      END IF;

      IF v_file_size IS NOT NULL AND v_file_size > 26214400 THEN
        RAISE EXCEPTION 'File size exceeds maximum allowed limit of 25MB.';
      END IF;

      v_stored_filename := COALESCE(NULLIF(v_file_name, ''), 'attachment');
      v_canonical_mime := public.get_canonical_mime_type(v_stored_filename);
      IF v_canonical_mime IS NULL THEN
        RAISE EXCEPTION 'Unsupported or restricted file extension for "%".', v_stored_filename;
      END IF;
      ...
      v_created_attachments := v_created_attachments || jsonb_build_array(jsonb_build_object(
        'id', v_att_id,
